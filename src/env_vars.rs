...
pub static ENV_VARS: &[(&str, &str)] = &[
    ...
    ("XEZIM_BUF_COLLAPSE", "Default on; =0 disables: collapse whole-net identity continuous assigns (assign y = x) onto their source net (drops one delta step per buffer); force/release targets, 2-state/4-state mismatches, SDF and DPI/VPI designs are left alone"),
    ...
    ("XEZIM_CLOCK_TREE", "Cycle engine: set to 0 to disable the eager clock-tree pass"),
    ("XEZIM_CLOCK_TREE_STATS", "Cycle engine: report the clock-tree conversion (roots, entries, unconverted derived clocks)"),
    ("XEZIM_CLOCK_TREE_TRACE", "Cycle engine: trace the tree walk for the named derived clocks (comma-separated)"),
    ...
    ("XEZIM_COMB_PATHS_DUMP", "Profiling: print the full instruction list of the top N ranked COMB_PATHS entries"),
    ...
    ("XEZIM_EDGE_MERGE", "Merge same-sensitivity edge blocks into one compiled block (default 8; =0 disables; =census reports; =N sets the chunk cap)"),
    ...
    ("XEZIM_EVALS_TOP", "Profiling: how many entries the COMB_PATHS evaluation census lists (default 40)"),
    ("XEZIM_EVENT_AFTER", "Debug: log event-queue state after each time step"),
    ("XEZIM_EVENT_EDGE", "Event-edge engine: force on/off (0/1)"),
    ("XEZIM_EVENT_EDGE_CENSUS", "Event-edge engine: report why edge blocks are not gateable and how many gate through array-element arming"),
    ...
    ("XEZIM_PROC_FSM_TS", "Processes: 0 keeps compiled process FSMs on the four-state VM instead of the two-state executor"),
    ...
    ("XEZIM_RANGE_COPY", "Comb settle: lower direct constant-range assignments to slice copies (1 enables)"),
    ...
    ("XEZIM_TS_JIT", "Two-state: compile straight-line streams to native code (jit feature; exploration)"),
    ...
    ("XEZIM_VEC_COALESCE", "Comb settle: coalesce aligned bit-sliced gates into vector operations (1 enables)"),
    ("XEZIM_VEC_SCATTER", "Comb settle: batch adjacent source lanes with scalar destinations (1 enables)"),
    ("XEZIM_VEC_SCATTER_MIN", "Comb settle: minimum scalar-destination batch width (2..64, default 64)"),
    ("XEZIM_VEC_STATS", "Comb settle: print vector-coalescing statistics"),
